...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const login = (returnTo: string = '/') => {
    ...
    // Open popup to our own login route, which will redirect to CAS
    // This ensures the popup stays on osdg.in domain
    const loginUrl = `/api/auth/cas/login?returnTo=${encodeURIComponent(returnTo)}`;
    
    console.log('[Auth] Opening popup to:', loginUrl);
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      console.log('[Auth] Message type:', event.data?.type);
      
      // Check if this is a CAS auth message
      if (!event.data || !event.data.type || !event.data.type.startsWith('CAS_AUTH')) {
        console.log('[Auth] Ignoring non-CAS message');
        return;
      }
      
      console.log('[Auth] ✅ Valid CAS message received');
